...
//print_r($_GET);exit();
$sql="SELECT * FROM  gh WHERE Id = '".$id."'";
...
while($row = mysqli_fetch_array($result)) {
	...
	if ($type == "GH")
	{
		...
		$phid = $row['phid'];
		$sql2="SELECT * FROM  ph WHERE Id = '".$phid."'";
		$result2 = mysqli_query($con,$sql2);
		$receiver="";
		if (mysqli_num_rows($result2))
		{
		...
		// receiver guider
		$sql3="SELECT * FROM  registration WHERE User_name='$receiver'";
		...
		$data15 = mysqli_fetch_array($result34);
		      $rguider_name = $data15['User_name'];	
		}

	}
?>
    
<div style="margin: 10px 0px; font-size: 17px;">Order Z<?php echo $row['GH_id']; ?></div>
<div style="margin: 10px 0px; font-size: 14px;">
<span>Participant of MMM has requested assistance in the amount of: <?php echo $row['ph_amount']." ".$row['Select_currency']; ?></span><br> 
<span style="font-weight: 600;">You have to provide help before <?php echo $row['GH_date']; ?>, according to BTC details provided further:</span>	
</div>
<div style="border-radius: 7px;padding: 7px;border: 1px solid gray;margin: 20px 0">
<div style="margin: 10px 0px; font-size: 14px;">
<h3><?php echo $btc_add; ?></h3>
...
<div style="margin: 20px 0px; color: red; font-size: 14px;">
Receiver: <?php echo $sender; ?> (*****)<br>
Reciever's guider: <?php echo $sguider; ?> (*****)<br>
Sender: <?php echo $receiver; ?><br>
Sender's guider: <?php echo $rguider_name; ?><br>
</div>
